...
(defun ymacs-cpp-cmake//render-env-widget (-buffer)
  ...
  (let* ((env (with-current-buffer -buffer (ymacs-cpp-cmake//config-env)))
         (widget (widget-create
                  'editable-list
                  :entry-format "%d %v"
                  :value env
    ...
    (let ((available-options
      ...
      (widget-insert
       (substitute-command-keys
        "(run `\\[ymacs-cpp-cmake/run-cmake]' to get available options)\n"))
...
(defun ymacs-cpp-cmake/config ()
  ...
  (let ((original-buffer (current-buffer))
        (buffer (get-buffer-create ymacs-cpp-cmake-config-buffer-name)))
