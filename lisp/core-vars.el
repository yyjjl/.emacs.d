...
(define-variable!
  xsel ;; Use `xsel' to copy/paste in terminal thoungh system clipboard
  fcitx ;; Whether to use X input method `fcitx'
  (mpv :exe "mplayer")
  (ripgrep :exe "rg")
  git
  aspell ;; Only one of `aspell' and `hunspell' is needed Use for spellcheck
  hunspell
  (gtags :exe "global") ;; Tag multiple languages
  latex)
