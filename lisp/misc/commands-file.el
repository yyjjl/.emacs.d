...
;;;###autoload
(defun ymacs-misc/copy-file-name (&optional -level -replace)
...
directory and extension."
  ...
  (set-transient-map
   (define-key! :map (make-sparse-keymap)
     ("=" . (lambda! (ymacs-misc/copy-file-name (min 6 (1+ -level)) t)))
     ("-" . (lambda! (ymacs-misc/copy-file-name (max 1 (1- -level)) t)))
     ("0" . (lambda! (ymacs-misc/copy-file-name 1 t))))))


;;;###autoload
(defun ymacs-misc/delete-this-file ()
  ...
  (when (yes-or-no-p (format "Really delete '%s'?"
    ...
    (run-hooks 'ymacs-misc-after-delete-this-file-hook)
    (kill-this-buffer)))

;;;###autoload
(defun ymacs-misc/copy-this-file-to-new-file ()
...
;;;###autoload
(defun ymacs-misc/rename-this-file-and-buffer (-new-name)
  ...
  (let ((filename (buffer-file-name)))
    (when (or (not (file-exists-p -new-name))
      ...
      (let ((inhibit-message t))
        (recentf-cleanup))
      (run-hook-with-args 'ymacs-misc-after-rename-this-file-hook filename -new-name)
