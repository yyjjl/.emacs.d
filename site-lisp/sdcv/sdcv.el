...
(defvar sdcv-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "n" 'sdcv-next-dictionary)
    (define-key map (kbd "C-p") 'sdcv-mode-previous-line)
    (define-key map "p" 'sdcv-previous-dictionary)
...
(defun sdcv-quit ()
  "kill sdcv process"
  (interactive)
  (sdcv-return-from-sdcv)
  (delete-process sdcv-process-buffer-name)
  (kill-buffer sdcv-buffer-name))
